"""Training/testing stages for visual-grounded CvT2DistilGPT2.

The only material difference from the supplied stages.py is strict=False when
loading warm-start checkpoints. This permits a baseline CvT2DistilGPT2
checkpoint to initialise the new model while leaving the newly added concept
...
def stages(args: Namespace):
    ...
    if args.train:
        ...
        if args.warm_start_ckpt_path:
            ...
            print(f"Warm-starting with non-strict loading: {args.warm_start_ckpt_path}.")

        elif getattr(args, "warm_start_exp_dir", None):
            warm_start_exp_dir_trial = os.path.join(
                args.warm_start_exp_dir, f"trial_{args.trial}"
            )
            ...
            print(f"Warm-starting with non-strict loading: {ckpt_path}.")
    ...
    if args.test:
        if args.fast_dev_run:
            ...
            if getattr(args, "other_exp_dir", None):
                other_exp_dir_trial = os.path.join(
                    args.other_exp_dir, f"trial_{args.trial}"
                )
                ckpt_path = get_test_ckpt_path(
                    other_exp_dir_trial,
                    args.other_monitor,
